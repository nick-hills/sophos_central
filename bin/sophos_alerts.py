	...
	if 200 <= connection.code <= 207:
		...
		for alert in alertList:
			created_at = str(alert['created_at'])
                        ...
                        type = str(alert['type'])
                        when = str(alert['when'])
                        print when + " customer_id=\""+customer_id+"\" description=\""+description+"\" id=\""+id+"\" location=\""+location+"\" name=\""+name+"\" severity=\""+severity+"\" eventsource=\""+source+"\" threat=\""+threat+"\" threat_cleanable=\""+threat_cleanable+"\" type=\""+type+"\" created_at=\""+created_at+"\" vendor=\"Sophos\" product=\"Central Endpoint Protection\""
